...
use std::{mem, thread::sleep, time::{Duration, Instant}};
use tokio::{
...
const TCP_ENDPOINT: &str = "127.0.0.1:3330";
const NUM_MSGS: usize = 100_000;
const MSG_SIZE: usize = 1024;
const BUSY_TIME: u64 = 1;
...
fn main() { 
    ...
    rt.spawn(async {
        ...
        while Instant::now() - start < Duration::from_millis(BUSY_TIME) {}
    ...
    rt2.spawn(async {
        ...
        let mut timing = Vec::<f64>::with_capacity(NUM_MSGS);
        for _ in 0..NUM_MSGS {
            ...
            let end = Instant::now();
            timing.push((end - start).as_micros() as f64);
        }
        tx.send(()).unwrap();

        let summary = Summary::new(timing.as_slice());
        println!("echo {:?}", summary)
    });
